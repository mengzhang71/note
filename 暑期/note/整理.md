...
![image-20231214095422662](%E6%95%B4%E7%90%86.assets/image-20231214095422662.png)
...
而你要写的方法，如果它是依赖新的数据，新的DOM结构的时候就可以nextTick，它的作用就是在下次 DOM 更新循环结束之后执行的传进来的延迟回调。
...
![image-20231220110444110](%E6%95%B4%E7%90%86.assets/image-20231220110444110.png)
...
```
  visualMap: {
        ...
        max,
        text: [
          `${max}(${this.value2 === '发生地质灾害' ? '起' : '个'})`,
          `${min}(${this.value2 === '发生地质灾害' ? '起' : '个'})`
        ],
        realtime: false,
        calculable: false,
        orient: 'horizontal',
...
```js
    this.areaMapChart.on('click', (params: any) => {
      const item = this.chartMapData[params.dataIndex]
      if (item) {
        const code = item.code
        const current = this.option1.find((it: any) => it.value === code)
        if (current) {
          this.areaCode = code
          this.params.xzqdm = code
          this.initData()
        }
      }
    })
...
拿到series中的数据，用dataIndex作为参考，去chartMapData映射的地图数据的详细，去匹配option1中的信息，改变传参，再去获取对应接口数据

![image-20231222171822061](%E6%95%B4%E7%90%86.assets/image-20231222171822061.png)
...
可以是 'plain'（普通图例）或者 'scroll'（可滚动的图例）
...
![image-20231221113316241](%E6%95%B4%E7%90%86.assets/image-20231221113316241.png)
...
```
git checkout .
git checkout project-省厅移动前端

git pull --rebase

git checkout -b dev-cockpit-20231218
